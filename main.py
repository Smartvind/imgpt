...
ENGINE = os.environ['GPT_ENGINE']
OPENAI_API = os.environ['OPENAI_API']
PROMPT = os.environ['PROMPT']
MATHPIX_APPID = os.environ['MATHPIX_APPID']
MATHPIX_KEY = os.environ['MATHPIX_KEY']
...
class Chatbot:
  ...
  def __init__(self,
    ...
    self.conversations = Conversation()  # dont need this for mvp
  ...
  def _get_completion(
      ...
      stream: bool = False,  #not streaming
  ):
    """
...
class Prompt:
  ...
  def __init__(self, buffer: int = None) -> None:
    """
        ...
        """
    self.base_prompt = (
      os.environ.get("CUSTOM_BASE_PROMPT") or
...
def main():
  ...
  st.title('Unlimited, Instant Solutions for Problem Sets')

  st.sidebar.markdown("# Examples")
  st.sidebar.image(Image.open("./example.png"))
  st.sidebar.image(Image.open("./example2.png"))

  uploaded_file = st.file_uploader(
    "Upload a a picture of problem set to get a solution. Only upload one question at a time. See examples in sidebar.",
    type=['png', 'jpeg'])
  if uploaded_file is not None:

    chatbot = Chatbot(api_key=OPENAI_API)
    ...
    with st.empty():
      try:
        for response in chatbot.ask_stream(prompt, temperature=0.8):
          re += response
          st.write(re)
      except:
        st.write("")
